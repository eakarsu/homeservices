...
export function psqlDatabaseUrl(value=process.env.DATABASE_URL){
  const url=new URL(value);
  // These pool settings belong to Prisma, not libpq/psql.
...
export function query(sql,{rows=false}={}){
  const args=[psqlDatabaseUrl(),'-v','ON_ERROR_STOP=1'];
...
export function executeScript(sql){
  const result=spawnSync('psql',[psqlDatabaseUrl(),'-v','ON_ERROR_STOP=1','-f','-'],{encoding:'utf8',input:sql});
...
export function persist(sql){
  return new Promise((resolve,reject)=>{
    const child=spawn('psql',[psqlDatabaseUrl(),'-v','ON_ERROR_STOP=1','-c',sql],{stdio:['ignore','ignore','pipe']});
    let error='';
