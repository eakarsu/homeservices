...
APP_PORT="${PORT:-3000}"
DB_NAME="home_services_ai"

echo "=========================================="
echo "  Home Services AI Platform - Startup Script"
echo "=========================================="
echo ""
...
export NEXTAUTH_SECRET="${NEXTAUTH_SECRET:-homeservices-dev-secret-change-in-production}"
...
# Remove query params from DATABASE_URL for psql (psql doesn't like ?schema=public)
PSQL_URL=$(echo "${DATABASE_URL}" | sed 's/\?.*$//')
USER_COUNT=$(psql "${PSQL_URL}" -t -c "SELECT COUNT(*) FROM \"User\";" 2>/dev/null | tr -d ' \n\r\t' || echo "error")
echo "User count result: '${USER_COUNT}'"
if [ "${USER_COUNT}" = "0" ] || [ "${USER_COUNT}" = "error" ] || [ -z "${USER_COUNT}" ]; then
...
echo ""
echo "=========================================="
echo "  Starting Home Services AI on port ${APP_PORT}"
echo "=========================================="
echo ""
echo "Access the application at: http://localhost:${APP_PORT}"
echo ""
echo "Test credentials:"
echo "  Admin: admin@homeservices.com / password123"
echo "  Dispatcher: dispatcher@homeservices.com / password123"
echo "  Technician: tech@homeservices.com / password123"
echo ""
